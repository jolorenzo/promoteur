...
# MEDTR (gff3 hétérogène avec n°accession dans colone ID: AC130806 en début et chromosome à la suite) (a ajouter la version phytozome9_0)
...
# POPTR (gff3: chr + scaff) (-) (à ajouter)
...
perl -ne 'if($_ =~/^(\d+)\t.+(\t.+\t.+\t.+\t.+\t.+\t.+\t.+)/){$label=$1;printf ("MAIZE%02d\tMGDB5b60$2\n",$label)} elsif ($_ =~/^(UNKNOWN)\t.+(\t.+\t.+\t.+\t.+\t.+\t.+\t.+)/){$label=$1;printf ("MAIZE%02d\tMGDB5b60$2\n",$label)}else{print $_}' /bank/genfam/MAIZE/MAIZE.gff3 > /bank/genfam/MAIZE/MAIZE-MGDB5b60-sequence_feature-genfam.gff3
