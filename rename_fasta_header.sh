...
#perl -ne 'if($_ =~/^>Bd_(\d+)/){$label=$1;printf (">BRADI%02d\n",$label)} else{print $_}' /bank/brachypodium_distachyon/Bdistachyon_192.fna > /bank/genfam/BRADI/BRADI-JGI1-chromosome-genfam.fna
#grep '>' /bank/genfam/BRADI/BRADI-JGI1-chromosome-genfam.fna
#perl -ne 'if($_ =~/^Bd(.+)\t.+(\t.+\t.+\t.+\t.+\t.+\t.+\t.+)/){$label=$1;printf ("BRADI%02d\tJGI1$2\n",$label)} elsif ($_ !~/^Bd.+\t.+\t.+\t.+\t.+\t.+\t.+\t.+\t.+\n/) {print OUT $_}else{print $_}' /bank/brachypodium_distachyon/Bdistachyon_192_gene_exons.gff3 > /bank/genfam/BRADI/BRADI-JGI1-sequence_feature-genfam.gff3
...
#perl -ne 'if($_ =~/^MtChr(.+)\t.+(\t.+\t.+\t.+\t.+\t.+\t.+\t.+)/){$label=$1;printf ("MEDTR%02d\tMt3.5v5$2\n",$label)} elsif ($_ !~/^MtChr.+\t.+\t.+\t.+\t.+\t.+\t.+\t.+\t.+\n/) {print OUT $_}else{print $_}' /bank/medicago_truncatula/Mtruncatula_198_gene_exons.gff3 > /bank/genfam/MEDTR/MEDTR-Mt3.5v5-sequence_feature-genfam.gff3
...
#perl -ne 'if($_ =~/^scaffold_(.+)\t.+(\t.+\t.+\t.+\t.+\t.+\t.+\t.+)/){$label=$1;printf ("POPTR_scaffold%04d\tJGI2$2\n",$label)} elsif ($_ !~/^scaffold_.+\t.+\t.+\t.+\t.+\t.+\t.+\t.+\t.+\n/) {print OUT $_}else{print $_}' /bank/populus_trichocarpa/Ptrichocarpa_210_gene_exons.gff3 > /bank/genfam/POPTR/POPTR-JGI2-sequence_feature-genfam.gff3
